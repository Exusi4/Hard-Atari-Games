...
from SILmodel2 import PrioritizedReplayBuffer
...
def train_model(args, device, output_size, model, rnd, optimizer, s_batch, target_ext_batch, target_int_batch, y_batch, adv_batch, next_obs_batch, old_action_probs):
    #buffer = PrioritizedReplayBuffer(num_samples, 0.6)
    #epoch = 3
...
def re_train_model(args, device, output_size, model, rnd, optimizer, s_batch, target_ext_batch, target_int_batch,
    y_batch, adv_batch, reward, next_obs_batch, old_action_probs):
    epoch = 1
    update_proportion = 0.25
    is_reward = reward > 0
    s_batch = torch.FloatTensor(s_batch[is_reward]).to(device)
    target_ext_batch = torch.FloatTensor(target_ext_batch[is_reward]).to(device)
    target_int_batch = torch.FloatTensor(target_int_batch[is_reward]).to(device)
    y_batch = torch.LongTensor(y_batch[is_reward]).to(device)
    adv_batch = torch.FloatTensor(adv_batch[is_reward]).to(device)
    next_obs_batch = torch.FloatTensor(next_obs_batch[is_reward]).to(device)
    ...
    with torch.no_grad():
        ...
        mask_reward = reward > 0
        action_probs_old_list = action_probs_old_list[mask_reward]
        m_old = Categorical(action_probs_old_list)
        log_prob_old = m_old.log_prob(y_batch)

    for i in range(epoch):
        with torch.no_grad():
            action_probs, value_ext, value_int = model(s_batch)
            adv_new = (target_ext_batch - value_ext.sum(1))
            adv_mask = adv_new > 0.2
            num_valid_samples = adv_mask.sum().item()
            if num_valid_samples <args.sil_porpotion * args.num_worker * num_step:
                return
            print(num_valid_samples)

        valid_s_batch = s_batch[adv_mask]
        valid_next_obs_batch = next_obs_batch[adv_mask]
        valid_y_batch = y_batch[adv_mask]
        valid_log_prob_old = log_prob_old[adv_mask]
        valid_adv_batch = adv_batch[adv_mask]
        valid_target_ext_batch = target_ext_batch[adv_mask]
        valid_target_int_batch = target_int_batch[adv_mask]

        if num_valid_samples > 128:
            sample_idx = np.random.randint(num_valid_samples, size=128)
        else:
            sample_idx = np.arange(num_valid_samples)
        #idx, weights, sil_idx = buffer.sample_batch(256)
        #if idx is not None:
        predict_next_state_feature, target_next_state_feature = rnd(valid_next_obs_batch[sample_idx])
        ...
        action_probs, value_ext, value_int = model(valid_s_batch[sample_idx])
        m = Categorical(action_probs)
        log_prob = m.log_prob(valid_y_batch[sample_idx])

        ratio = torch.exp(log_prob - valid_log_prob_old[sample_idx])

        surr1 = ratio * valid_adv_batch[sample_idx].clamp(min=0, max=1)
        surr2 = torch.clamp(
            ratio,
            1.0 - args.eps,
            1.0 + args.eps) * valid_adv_batch[sample_idx].clamp(min=0, max=1)

        actor_loss = -torch.min(surr1, surr2).mean()
        critic_ext_loss = F.mse_loss(value_ext.sum(1), valid_target_ext_batch[sample_idx])
        critic_int_loss = F.mse_loss(value_int.sum(1), valid_target_int_batch[sample_idx])
        ...
        optimizer.zero_grad()
        loss = 0.5 * (actor_loss + 0.5 * critic_loss - args.entropy_coef * entropy + forward_loss)
        loss.backward()
        global_grad_norm_(list(model.parameters())+list(rnd.predictor.parameters()))
        optimizer.step()
...
def main():
    ...
    print('Training...')
    running_reward = 0
    acc_reward = np.zeros(args.num_worker)
    while True:
        ...
        reward_mask = np.zeros([args.num_worker, args.num_step])
        for n in range(args.num_worker):
            for t in range(args.num_step):
                acc_reward[n] += total_reward[n, t]
                if total_done[n, t]:
                    running_reward = (1/args.num_worker) * acc_reward[n] + (1-args.num_worker) * running_reward
                    acc_reward[n] = 0
                if acc_reward[n] > running_reward + args.sil_reward_add and total_reward[n, t] > 0:
                    reward_mask[n, :t+1] = 1
        reward_mask = reward_mask.reshape([-1])

        print(running_reward)
        if np.sum(reward_mask) > 0:
            re_train_model(args, device, output_size, model, rnd, optimizer,
                            np.float32(total_state) / 255., ext_target, int_target, total_action,
                            total_adv, reward_mask, total_next_obs.clip(-5, 5),
                            total_action_probs)
