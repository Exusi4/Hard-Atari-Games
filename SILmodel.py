...
class SILModel:
    def __init__(self, model, args, optimizer, device,
        sil_capacity=5000, batch_size = 268, min_batch_size =32,
        sil_alpha=0.6, sil_beta=0.1, sil_n_update=2, w_value=0.01):
        self.args = args
    ...
    def train(self):
        ...
        for n in range(self.sil_n_update):
            ...
            if states is not None:
                ...
                entropy = m.entropy().view(-1, 1)
                log_prob = -m.log_prob(actions).view(-1, 1)
                clipped_log_prob = log_prob.clamp(max=100)
